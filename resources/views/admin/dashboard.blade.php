...
@section('content')
    ...
    <div class="grid grid-cols-2 lg:grid-cols-4 gap-4 md:gap-6 mt-8 stagger-children" data-reveal="stagger">
        <div class="bg-white rounded-2xl p-5 md:p-6 shadow-sm hover:shadow-md transition-all duration-300 hover:-translate-y-0.5">
            ...
            <p class="text-3xl font-bold" style="color: #124170;" data-counter data-target="{{ $totalProducts }}" data-duration="800">{{ $totalProducts }}</p>
        ...
        <div class="bg-white rounded-2xl p-5 md:p-6 shadow-sm hover:shadow-md transition-all duration-300 hover:-translate-y-0.5">
            ...
            <p class="text-3xl font-bold" style="color: #124170;" data-counter data-target="{{ $totalCategories }}" data-duration="800">{{ $totalCategories }}</p>
        ...
        <div class="bg-white rounded-2xl p-5 md:p-6 shadow-sm hover:shadow-md transition-all duration-300 hover:-translate-y-0.5">
            ...
            <p class="text-3xl font-bold" style="color: #124170;" data-counter data-target="{{ $totalBestSellers }}" data-duration="800">{{ $totalBestSellers }}</p>
        ...
        <div class="bg-white rounded-2xl p-5 md:p-6 shadow-sm hover:shadow-md transition-all duration-300 hover:-translate-y-0.5">
            ...
            <p class="text-3xl font-bold" style="color: #124170;" data-counter data-target="{{ $totalNewArrivals }}" data-duration="800">{{ $totalNewArrivals }}</p>
    ...
    <div data-reveal="fade-up" data-reveal-delay="300" class="mt-8">
        <div class="bg-white rounded-2xl shadow-sm p-6 md:p-8">
            ...
            <div class="space-y-5">
                @foreach($productsPerCategory as $index => $cat)
                    <div data-reveal="fade-up" data-reveal-delay="{{ 50 * $index }}">
                        <div class="flex items-center justify-between mb-2">
                            <span class="text-sm font-medium text-gray-700">{{ $cat->name }}</span>
                            <div class="flex items-center gap-3">
                                <span class="text-sm font-semibold" style="color: #124170;">{{ $cat->products_count }}</span>
                                <span class="text-xs text-gray-400">({{ $totalProducts > 0 ? round(($cat->products_count / $totalProducts) * 100) : 0 }}%)</span>
                            </div>
                        </div>
                        <div class="w-full h-2.5 rounded-full bg-gray-100 overflow-hidden">
                            <div class="h-full rounded-full progress-bar-animate" style="width: {{ $totalProducts > 0 ? ($cat->products_count / $totalProducts) * 100 : 0 }}%; background: linear-gradient(90deg, #AAFFC7, #67C090);"></div>
                        </div>
                    </div>
                @endforeach
            </div>
