...
@section('content')
    ...
    <div class="mt-6 bg-white rounded-2xl shadow-sm p-6 md:p-8" data-reveal="fade-up" data-reveal-delay="150">
        <form action="{{ route('admin.products.update', $product) }}" method="POST" class="max-w-2xl space-y-6">
            ...
            <div data-reveal="fade-up" data-reveal-delay="450">
                <label class="block text-sm font-medium text-gray-700 mb-3">Preview Gambar</label>
                <img src="{{ $product->image }}" alt="{{ $product->name }}" class="w-32 h-32 rounded-xl object-cover border border-gray-200 hover:scale-105 transition-transform duration-300">
            </div>

            <div class="flex items-center gap-8" data-reveal="fade-up" data-reveal-delay="500">
                <label class="flex items-center gap-2.5 cursor-pointer group">
                    <input type="checkbox" name="is_best_seller" value="1" @checked(old('is_best_seller', $product->is_best_seller)) class="w-4 h-4 rounded border-gray-300 text-[#124170] focus:ring-[#124170] transition-all">
                    <span class="text-sm font-medium text-gray-700 group-hover:text-gray-900 transition-colors">Best Seller</span>
                </label>
                <label class="flex items-center gap-2.5 cursor-pointer group">
                    <input type="checkbox" name="is_new_arrival" value="1" @checked(old('is_new_arrival', $product->is_new_arrival)) class="w-4 h-4 rounded border-gray-300 text-[#124170] focus:ring-[#124170] transition-all">
                    <span class="text-sm font-medium text-gray-700 group-hover:text-gray-900 transition-colors">New Arrival</span>
                </label>
            </div>

            <div class="flex gap-3 pt-4" data-reveal="fade-up" data-reveal-delay="550">
